...
np.random.seed(1)
#use HDP to sample the socialbilities
...
class SparseHDPLink:
    ...
    def __init__(self, ind, y, nvec, R1, R2, m, B, device, jitter=1e-3):
        self.device = device
        self.ind = ind
    ...
    def nELBO_batch(self, sub_ind):
        ...
        #B \times K*R_1
        #U_omega = torch.cat([log_omega[k][self.ind[sub_ind, k],:] for k in range(self.nmod)], 1)
        U_omega = torch.cat([self.omega_hat[k][self.ind[sub_ind, k],:] for k in range(self.nmod)], 1)
        #U_omega = self.bn(U_omega) #batch-normalization over log-omega
        U_theta = torch.cat([torch.sigmoid(self.theta[k][self.ind[sub_ind, k],:]) for k in range(self.nmod)], 1)
        #U_theta = torch.cat([self.theta[k][self.ind[sub_ind, k],:] for k in range(self.nmod)], 1)
        #B \times K*R
        input_emb = torch.cat([U_omega, U_theta], 1)
        #input_emb = self.bn(input_emb)
    ...
    def pred(self, test_ind):
        #(d_k+1) \times R_1 in each mode k
        #log_omega = [torch.log_softmax(self.omega_hat[k] , dim = 0) for k in range(self.nmod)]
        #U_omega = torch.cat([log_omega[k][test_ind[:, k],:] for k in range(self.nmod)], 1)
        ...
        #U_theta = torch.cat([self.theta[k][test_ind[:, k],:] for k in range(self.nmod)], 1)
    ...
    def pred_link(self, test_ind):
        log_omega = [torch.log_softmax(self.omega_hat[k], dim = 0) for k in range(self.nmod)]
        s = torch.sum(torch.stack([log_omega[k][test_ind[:, k],:] for k in range(self.nmod)]), 0)
        edge_prob = 1/self.R1 * torch.sum(torch.exp(s), dim=1)
        return edge_prob

    def _callback(self, ind_link, yte_link):
        #self.bn.eval()
        with torch.no_grad():
            tau = torch.exp(self.log_tau)
    ...
    def train(self, ind_te, yte, lr, max_epochs=100):
        ...
        #paras = [self.log_alpha] + self.beta_hat + self.omega_hat + [self.log_gamma] + [self.Z, self.mu, self.L,  self.log_tau] + [p for p in self.bn.parameters()]
        paras = self.beta_hat + self.omega_hat + [self.log_gamma] + [self.Z, self.mu, self.L, self.log_tau] #+ [p for p in self.bn.parameters()]
